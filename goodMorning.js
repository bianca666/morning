(function(){
	window.supportsTouches = ("ontouchstart" in document);
	window.currDragged = {};
	window.dropAreas = null;

	function Leo(x, y, width, height){
		this.width = width;
		this.height = height;
		this.x = x;
		this.y = y;
		...
		this.initDiv = function() {
			...
			this.div.id = 'leo';
			this.div.style.width = this.width ? this.width + 'px' : '392px';
			this.div.style.height = this.height ? this.height + 'px' : '478px';
			this.div.style.position = 'absolute';
			this.div.style.left = this.x ? this.x + this.width/2 + 'px' : '50%';
			this.div.style.top = this.x ? this.x + this.height/2 + 'px' : '50%';
		...
		this.initDraggedItems = function() {
			...
			for([key, value] of opeArr){
				...
				divOpe.style.position = 'relative';
		...
		this.initDragEvent = function(){
			...
			if(!supportsTouches){            /*desktop*/
				...
				document.addEventListener("dragend", (e) => handleDragEnd(e, currDragged, this), false);
				...
				document.ontouchstart = (e) => {
					e.preventDefault();	
					/*window.currDragged.startPoint = getMousePoint(e);
					window.currDragged.item = e.target;
					window.currDragged.offsetX = e.offsetX;
					window.currDragged.offsetY = e.offsetY;
					window.dropAreas = this.dropAreas;*/
					...
					document.ontouchmove = (e) => {
						handleMoveEvent(e, currDragged, this.dropAreas, this);
					}
			...
			/*document.addEventListener('dragstart', function(e) {
				...
				//e.dataTransfer.setData('text/plain', 'foo=bar')	;
				window.currDragged.startPoint = getMousePoint(e);
				window.currDragged.item = e.target;
				window.currDragged.offsetX = e.offsetX;
				window.currDragged.offsetY = e.offsetY;
				console.log(window.currDragged);
				document.addEventListener('drag', (e) => console.log('drag'), false);
			}, false);

			

			document.addEventListener(endEvent, handleEndEvent, false);*/
		...
		function handleDragEnd(e, currDragged, that){
		
			
			that.body['lEyebrow'].classList.remove("dragover");
			that.body['rEyebrow'].classList.remove("dragover");
			that.body["lEye"].classList.remove("dragover");
			that.body["rEye"].classList.remove("dragover");
			that.body['mouth'].classList.remove("dragover");
			that.body['nose'].classList.remove("dragover");
			that.body['head'].classList.remove("dragover");
			that.body['neck'].classList.remove("dragover");
		...
		function handleDrag(e, currDragged, dropAreas, thisRef){
			...
			currDragged.movePoint = getMousePoint(e);
			currDragged.item.style.transform = 'translate(' + (currDragged.movePoint.x - currDragged.startPoint.x )+ 'px, ' + (currDragged.movePoint.y - currDragged.startPoint.y )+ 'px)';
			currDragged.draggedImage.style.left = e.clientX - currDragged.offsetX  + 'px' ;
			currDragged.draggedImage.style.top = e.clientY - currDragged.offsetY  + 'px' ;
			hoverEffect(e, currDragged, dropAreas, thisRef)
		}
			

		function handleMoveEvent(e, currDragged, dropAreas, thisRef){
			
			e.preventDefault();
			
			//var currDragged = window.currDragged;
			currDragged.movePoint = getMousePoint(e);
		...
		function hoverEffect(e, currDragged, dropAreas, thisRef){
		...
		function handleEndEvent(e){
			...
			document.removeEventListener(endEvent, handleEndEvent, false);
			document.removeEventListener(moveEvent, handleMoveEvent, false);
		...
		function handleDragStart(e, currDragged, opeArr){	
			...
			currDragged.draggedImage.style.left = currDragged.startPoint.x - currDragged.offsetX + 'px';
			currDragged.draggedImage.style.top = currDragged.startPoint.y - currDragged.offsetY + 'px';
		...
		function insideDropArea(e, currDragged, dropAreas){
			var xR = currDragged.movePoint.x /*e.clientX*/ - currDragged.offsetX + currDragged.item.offsetWidth;
			
			var xL = currDragged.movePoint.x - currDragged.offsetX;
			var yR = currDragged.movePoint.y - currDragged.offsetY + currDragged.item.offsetHeight;
			var yL = currDragged.movePoint.y - currDragged.offsetY;



			return dropAreas.some(function(area){
				//console.log(xL, xR, yL, yR, area.x, area.rx, area.y, area.ry, area.id);
		...
		this.showOpeAnim = function() {
			switch(this.currDragged.item.id){
				...
				var opeKeys = this.opeArr.keys();
				for(let o of opeKeys) {
					if( o == this.currDragged.item.id){
						continue;
					}
				...
				/*while(!i.done){
					...
					if(i.value === this.currDragged.item.id){
						i = firstValue.next();
					
						continue;
					}else{	
						document.getElementById(i.value).setAttribute('draggable', 'false');
						i = firstValue.next();
					}
		...
		function handleDrop(e, currDragged, thisRef){
			...
			if(currDragged.inside){	
				...
				switch(currDragged.item.id){
					case "wfOpe":
						thisRef.washFace();
						setTimeout(() => { currDragged.item.style.opacity = 1; currDragged.item = null;resDraggble(); currDragged.inside = false;}, 9740);
						break;
					case "btOpe":
						thisRef.brushTeeth();
						setTimeout(() => { currDragged.item.style.opacity = 1; currDragged.item = null; currDragged.inside = false; resDraggble()},
							8000);
						break;
					case "ebOpe":
				...
				function resDraggble(){
					var firstValue = thisRef.opeArr.keys();
					var i = firstValue.next();
					while(!i.done){	
						document.getElementById(i.value).setAttribute('draggable', 'true');
